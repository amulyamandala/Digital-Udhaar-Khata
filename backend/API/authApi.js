...
authApp.post("/register",async(req,res)=>{
  try{
    ...
    // create user
    const newUser=await UserModel.create({name,phone,shopName,language,password: hashedPassword,subscriptionPlan});
...
authApp.post("/login",async(req,res)=>{
  try {
    ...
    // access token
    const token = sign({id: user._id,phone: user.phone},process.env.JWT_SECRET,{expiresIn: "2d"});
    // refresh token
